...
// DRR rounded to 2 decimal places max
...
function getHealth(doc) {
  if (doc === null || doc === undefined) return { label: 'N/A',            cls: 'badge-gray' };
  if (doc <= 120) return { label: '✅ Healthy',         cls: 'badge-healthy' };
  if (doc <= 150) return { label: '⚠ Unhealthy',       cls: 'badge-slow' };
  if (doc <= 180) return { label: '🔴 Very Unhealthy',  cls: 'badge-over' };
  return           { label: '💀 Dead Inventory',        cls: 'badge-dead' };
}

// Portal-wise PO status logic (points 6-9)
function getPortalStatus(portalDOC, openPO) {
  if (portalDOC === null || portalDOC === undefined) return null;
  const hasPO = openPO > 0;
  if (!hasPO && portalDOC < 30) return { label: 'PO Required',    cls: 'badge-po',       priority: 1 };
  if (hasPO  && portalDOC < 7)  return { label: '🚨 Please Send', cls: 'badge-critical', priority: 4 };
  if (hasPO  && portalDOC < 15) return { label: '🔴 Critical',    cls: 'badge-critical', priority: 3 };
  if (hasPO  && portalDOC < 30) return { label: '🟠 Urgent PO',   cls: 'badge-urgent',   priority: 2 };
  return null;
}
...
const DEFAULT_VISIBLE = new Set(['link','sku','title','supplier','category','whInv','amzInv','flkInv','zptInv','blkInv','amzDRR','flkDRR','zptDRR','blkDRR','whDOC','amzDOC','flkDOC','zptDOC','blkDOC','openPO','totalInv','totalDRR','companyDOC','health','suggestQty','action']);
...
export default function AllProductsPage({ initialFilter }) {
  ...
  const rows = useMemo(() => {
    ...
    else if (fAlert === 'over')  r = r.filter(x => x.companyDOC !== null && x.companyDOC > 120 && x.companyDOC <= 180);
    return r;
  }, [allRows, search, fSupplier, fCategory, fAlert]);
  ...
  const exportCSV = () => {
    ...
    const body = rows.map(r => {
      const health = getHealth(r.companyDOC);
      return visibleCols.map(c => {
        switch(c.key) {
          ...
          case 'whInv':      return r.whInv || 0;
          case 'amzInv':     return r.amzInv || 0;
          case 'flkInv':     return r.flkInv || 0;
          case 'zptInv':     return r.zptInv || 0;
          case 'blkInv':     return r.blkInv || 0;
          case 'amzDRR':     return fmtDRR(r.amzDRR);
          case 'flkDRR':     return fmtDRR(r.flkDRR);
          case 'zptDRR':     return fmtDRR(r.zptDRR);
          case 'blkDRR':     return fmtDRR(r.blkDRR);
          case 'whDOC':      return r.whDOC?.toFixed(1) || '';
          ...
          case 'mfgQty':     return r.mfgQty || 0;
          case 'totalInv':   return r.totalInv || 0;
          ...
          case 'action':     return r.actionRequired || '';
          default: return '';
        }
      }).join(',');
    });
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-ghost" onClick={() => setShowColPicker(!showColPicker)}>⚙ Columns ({visible.size})</button>
      ...
      {showColPicker && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: 16, marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          <div style={{ width: '100%', fontWeight: 600, fontSize: 12, marginBottom: 4 }}>Select columns to show/export:</div>
          {ALL_COLUMNS.map(col => (
          ...
          <button className="btn btn-ghost btn-xs" onClick={() => setVisible(new Set(ALL_COLUMNS.map(c => c.key)))}>Select All</button>
      ...
      <div className="filter-row" style={{ marginBottom: 12 }}>
        ...
        <select className="filter-select" value={fAlert} onChange={e => setFAlert(e.target.value)}>
          ...
          <option value="low">All Low DOC</option>
          <option value="dead">Dead Stock</option>
          <option value="over">Overstock</option>
        </select>
        {(search || fSupplier !== 'all' || fCategory !== 'all' || fAlert !== 'all') && (
          <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(''); setFSupplier('all'); setFCategory('all'); setFAlert('all'); }}>✕ Clear</button>
        )}
      ...
      {!rows.length ? <Empty icon="🔍" title="No products found" desc="Try clearing filters." /> : (
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                {ALL_COLUMNS.filter(c => visible.has(c.key)).map(c => <th key={c.key}>{c.label}</th>)}
              </tr>
            </thead>
            <tbody>
              {rows.map(r => {
                ...
                return (
                  <tr key={r.asin}>
                    ...
                    {visible.has('sku')        && <td style={{ color:'var(--muted)', fontSize:11 }}>{r.sku||'—'}</td>}
                    {visible.has('title')      && <td style={{ maxWidth:180, overflow:'hidden', textOverflow:'ellipsis', fontSize:11 }} title={r.title}>{r.productLink ? <a href={r.productLink} target="_blank" rel="noreferrer" style={{ color:'var(--blue)', textDecoration:'none' }}>{r.title||'—'}</a> : r.title||'—'}</td>}
                    ...
                    {visible.has('whInv')      && <td style={{ fontWeight:500 }}>{fmtN(r.whInv)}</td>}
                    ...
                    {visible.has('health')     && <td><span className={`badge ${health.cls}`}>{health.label}</span></td>}
                    {visible.has('suggestQty') && <td style={{ color:'var(--blue)', fontWeight:600 }}>{r.suggestQty>0?fmtN(r.suggestQty):'—'}</td>}
                    {visible.has('action')     && <td><span className={r.actionRequired==='need_po'?'action-need':r.actionRequired==='stock_ok'||r.actionRequired==='no_need'?'action-ok':'action-stop'}>{r.actionRequired==='need_po'?'⚡ Need PO':r.actionRequired==='no_need'?'✓ No Need':r.actionRequired==='stock_ok'?'✓ Stock OK':r.actionRequired==='liquidate'?'🔻 Liquidate':'⛔ Overstock'}</span></td>}
                  </tr>
                );
              })}
            </tbody>
