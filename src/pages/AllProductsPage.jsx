  ...
  return (
    <div>
      ...
      <div className="filter-row" style={{ marginBottom: 12 }}>
        ...
        <select className="filter-select" value={fCategory} onChange={function(e) { setFCategory(e.target.value); }}>
        ...
        {(search || fSupplier !== 'all' || fCategory !== 'all' || fAlert !== 'all' || fAction !== 'all') && (
          <button className="btn btn-ghost btn-sm" onClick={function() { setSearch(''); setFSupplier('all'); setFCategory('all'); setFAlert('all'); setFAction('all'); }}>Clear</button>
        )}
      ...
      {rows.length === 0 ? <Empty icon="🔍" title="No products found" desc="Try clearing filters." /> : (
        <div className="table-wrap" style={{ overflow: 'auto', maxHeight: '75vh' }}>
          <table style={{ borderCollapse: 'separate', borderSpacing: 0 }}>
            <thead>
              <tr>
                <th style={{ width: 32, position: 'sticky', left: 0, top: 0, zIndex: 5, background: 'var(--card)' }}>
