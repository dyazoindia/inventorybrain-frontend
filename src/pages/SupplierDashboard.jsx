...
// Supplier config — add new suppliers here in future
var SUPPLIER_CONFIG = {
  CHINA: {
    label:       'China Supplier',
    icon:        '🏭',
    color:       '#7c3aed',
    heroClass:   'hero-china',
    docThreshold: 120,
    whThreshold:  60,
    targetDOC:    120
  },
...
export default function SupplierDashboard({ supplier }) {
  ...
  var confirmMut = useMutation({
    ...
    onSuccess: function(data) {
      ...
      qc.invalidateQueries(['pos-' + supplier]);
      qc.invalidateQueries(['inventory-' + supplier]);
  ...
  var doExport = function() {
    exportToCSV(sel.count > 0 ? sel.selectedRows : rows, [
      ...
      { key: 'companyDOC',   label: 'Co. DOC',      getValue: function(r) { return fmtDoc(r.companyDOC); } },
    ...
    <div>
      ...
      <div className={'hero ' + config.heroClass} style={{ marginBottom: 16 }}>
        ...
        <p>
          ...
          {counts.manufacturing} in manufacturing &nbsp;·&nbsp;
      ...
      <div className="info-box" style={{ marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', fontSize: 11 }}>
        ...
        <span className="badge badge-confirmed">2. Supplier Confirms → Mfg Inv</span>
        <span style={{ color: 'var(--muted)' }}>→</span>
      ...
      <div className="info-box" style={{ marginBottom: 14, fontSize: 11 }}>
        <strong>Logic:</strong> WH DOC &lt;{config.whThreshold}d = flag &nbsp;|&nbsp;
        Co. DOC &lt;{config.docThreshold}d = Need PO &nbsp;|&nbsp;
        Co. DOC &gt;{config.docThreshold}d = No Need &nbsp;|&nbsp;
        Target DOC = {config.targetDOC}d
      </div>
          ...
          return (
            <button key={tab.key} onClick={function() { setActiveTab(tab.key); }}
              style={{
                ...
                background: active ? tab.color : 'var(--bg3)',
              ...
              {cnt > 0 && (
                <span style={{
                  marginLeft: 5,
                  background: active ? 'rgba(255,255,255,.25)' : tab.color + '20',
      ...
      {rows.length === 0 ? (
        <Empty icon={config.icon}
          title={'No products in ' + TABS.find(function(t) { return t.key === activeTab; }).label}
          desc="Try a different tab or check back after uploading data." />
      ) : (
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style={{ background: '#e0f2fe', color: '#0891b2', fontWeight: 700 }}>
                  Mfg Qty
                </th>
                <th>Confirm Type</th>
                <th>Confirmed Qty</th>
              ...
              {rows.map(function(r) {
                ...
                return (
                  <tr key={r.asin} style={{ background: sel.selected.has(r.asin) ? 'var(--blue-lt)' : '' }}>
                    ...
                    <td>
                      {r.productLink
                        ? <a className="link-btn" href={r.productLink} target="_blank" rel="noreferrer">Link</a>
                        : '\u2014'}
                    </td>
                    ...
                    {/* Title */}
                    <td style={{ maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', fontSize: 11 }}>
                      {r.title || '\u2014'}
                    </td>
                    ...
                    {/* WH DOC */}
                    <td>
                      <span style={{ fontWeight: 600, color: docColor(r.whDOC) }}>{fmtDoc(r.whDOC)}</span>
                    </td>

                    {/* Co. DOC */}
                    <td>
                      <span style={{ fontWeight: 700, color: docColor(r.companyDOC) }}>{fmtDoc(r.companyDOC)}</span>
                    </td>
                    ...
                    {/* Final Qty — Admin sets, supplier reads */}
                    ...
                    {/* Mfg Qty — auto-filled when supplier confirms */}
                    <td style={{ background: '#e0f2fe', fontWeight: 600, color: r.mfgQty > 0 ? '#0891b2' : 'var(--muted)', textAlign: 'center' }}>
                      {r.mfgQty > 0 ? fmtN(r.mfgQty) : '\u2014'}
                    </td>

                    {/* Confirm Type — supplier action when status is admin_approved */}
                    <td>
                      {po && po.status === 'admin_approved' && !isAdmin ? (
                        <select className="filter-select" style={{ fontSize: 11, padding: '3px 6px' }}
                          value={ci.type}
                          onChange={function(e) {
                            var n = {}; n[poId] = { type: e.target.value, qty: e.target.value === 'full' ? po.finalQty : ci.qty };
                        ...
                        <span style={{ fontSize: 11, color: 'var(--muted)' }}>
                          {po && po.poConfirmType ? po.poConfirmType : '\u2014'}
                        </span>
                      )}
                    </td>

                    {/* Confirmed Qty — supplier enters and clicks confirm */}
                    <td>
                      {po && po.status === 'admin_approved' && !isAdmin ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {ci.type === 'custom' && (
                            <input type="number" min="0" max={po.finalQty} value={ci.qty}
                              onChange={function(e) {
                                var n = {}; n[poId] = { type: ci.type, qty: e.target.value };
                                setConfirmInputs(Object.assign({}, confirmInputs, n));
                              }}
                              style={{ width: 70, padding: '3px 6px', border: '1px solid var(--border)', borderRadius: 5, fontSize: 12, textAlign: 'center' }} />
                          )}
                          <button className="btn btn-primary btn-xs"
                            onClick={function() { confirmMut.mutate({ id: po._id, type: ci.type, qty: parseInt(ci.qty) || po.finalQty }); }}
                        ...
                        <span style={{ fontWeight: 600, color: 'var(--blue)' }}>
                          {po && po.confirmedQty ? fmtN(po.confirmedQty) : '\u2014'}
                        </span>
                      )}
                    </td>

                    {/* Shipped Qty — supplier enters when confirmed */}
                    <td>
                      {po && po.status === 'supplier_confirmed' && !isAdmin ? (
                        <div style={{ display: 'flex', gap: 4 }}>
                          <input type="number" min="0" max={po.confirmedQty} value={si}
                            ...
                            onChange={function(e) {
                              var n = {}; n[poId] = e.target.value;
                              setShipInputs(Object.assign({}, shipInputs, n));
                            }}
                            style={{ width: 70, padding: '3px 6px', border: '1px solid var(--border)', borderRadius: 5, fontSize: 12, textAlign: 'center' }} />
                          <button className="btn btn-success btn-xs"
                            onClick={function() { shipMut.mutate({ id: po._id, qty: parseInt(si) || po.confirmedQty }); }}
                            disabled={shipMut.isPending}>Ship</button>
                        </div>
                      ) : (
                        <span style={{ fontWeight: 600, color: 'var(--teal)' }}>
                          {po && po.shippedQty ? fmtN(po.shippedQty) : '\u2014'}
                        </span>
                      )}
                    </td>

                    {/* Delivered — Admin only, auto-updates warehouse */}
                    <td>
                    ...
                    {/* Action Required */}
                    <td>
                      {r.actionType === 'supplier_po_required' && (
                        <span className="action-need">Need PO</span>
                      )}
                      {r.actionType === 'supplier_po_inprogress' && (
                        <span className="badge badge-confirmed">In Progress</span>
                      )}
                      {r.actionType === 'no_action' && (
                        <span className="action-ok">No Action</span>
                      )}
                      {!r.actionType && <span className="badge badge-gray">\u2014</span>}
                    </td>
                  </tr>
                );
              })}
